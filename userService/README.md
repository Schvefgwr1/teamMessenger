...
- Интеграционные тесты (с поднятием окружения, миграциями и подробным выводом):
  ```bash
  cd userService
  make integration
  ```
  По умолчанию запускается `go test -tags=integration -v ./tests/integration/...`.
  Ограничить набор тестов: `make integration TEST_ARGS="-v -run TestAuthFlow"`.

- Окружение интеграций: `tests/integration/docker-compose.integration.yml`
  - Postgres доступен на `localhost:5433` (DB_NAME `team_messenger_test`, user/password `postgres`)
  - Kafka/Zookeeper без постоянных volumes; данные очищаются после `make integration`
  - При старте дропается схема `user_service` и применяются миграции