...
func cleanupTestData(t *testing.T, db *gorm.DB) {
	...
	db.Exec("DELETE FROM chat_service.message_files WHERE message_id IN (SELECT id FROM chat_service.messages WHERE content LIKE 'test_%')")
	db.Exec("DELETE FROM chat_service.messages WHERE content LIKE 'test_%'")
	// Удаляем тестовые чаты и связанные данные
	db.Exec("DELETE FROM chat_service.chat_user WHERE chat_id IN (SELECT id FROM chat_service.chats WHERE name LIKE 'test_%')")
	db.Exec("DELETE FROM chat_service.chats WHERE name LIKE 'test_%'")
	// Роли и permissions не удаляем, так как они нужны для тестов
}
